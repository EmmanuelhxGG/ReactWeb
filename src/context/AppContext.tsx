...
import {
  ...
  updateProductReview as apiUpdateProductReview,
  deleteProductReview as apiDeleteProductReview
...
const PRODUCT_COMMENT_PREFIX = "product:";
...
type CustomerRegistrationPayload = Omit<CustomerUser, "createdAt" | "bdayRedeemedYear" | "id" | "status" | "prefs"> & {
...
const mapCustomerSummary = (dto: CustomerSummaryDto, previous?: CustomerUser): CustomerUser => {
  const prevPrefs = previous?.prefs;
  const nextDefaultShip = dto.defaultShippingCost ?? prevPrefs?.defaultShip ?? DEFAULT_SHIPPING_COST;
  const nextNewsletter = dto.newsletter ?? prevPrefs?.newsletter ?? false;
  const nextSaveAddress = dto.saveAddress ?? prevPrefs?.saveAddress ?? false;

  return {
    id: dto.id,
    run: dto.run ?? previous?.run ?? "",
    tipo: dto.customerType ?? previous?.tipo ?? "Cliente",
    nombre: dto.firstName ?? previous?.nombre ?? "",
    apellidos: dto.lastName ?? previous?.apellidos ?? "",
    email: dto.email ?? previous?.email ?? "",
    fnac: dto.birthDate ?? previous?.fnac ?? "",
    region: dto.region ?? previous?.region ?? "",
    comuna: dto.commune ?? previous?.comuna ?? "",
    direccion: dto.address ?? previous?.direccion ?? "",
    phone: dto.phone ?? previous?.phone ?? undefined,
    createdAt: previous?.createdAt ?? Date.now(),
    promoCode: dto.promoCode ?? previous?.promoCode ?? undefined,
    felices50: dto.felices50 ?? previous?.felices50 ?? false,
    bdayRedeemedYear: dto.birthdayRedeemedYear ?? previous?.bdayRedeemedYear ?? undefined,
    prefs: prevPrefs
      ? {
          ...prevPrefs,
          defaultShip: nextDefaultShip,
          newsletter: nextNewsletter,
          saveAddress: nextSaveAddress
        }
      : {
          defaultShip: nextDefaultShip,
          newsletter: nextNewsletter,
...
const toCustomerRegistrationRequest = (payload: CustomerRegistrationPayload): CustomerRegistrationRequestDto => {
...
type ContextValue = {
  ...
  registerCustomer: (payload: CustomerRegistrationPayload) => Promise<{ ok: boolean; message?: string }>;
  createCustomerAsAdmin: (payload: CustomerRegistrationPayload) => Promise<{ ok: boolean; message?: string }>;
  ...
  try {
    ...
    if (parsed && typeof parsed === "object" && typeof parsed.token === "string" && parsed.token) {
      return {
        ...
        status: typeof parsed.status === "string" ? normalizeAccountStatus(parsed.status) : null
      };
    }
  } catch (error) {
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const inactiveNoticeShownRef = useRef(false);
  ...
  const hydrateForAuth = useCallback(async (state: AuthState) => {
    ...
    if (state.role === "CUSTOMER") {
      ...
      try {
        ...
        setCustomerProfile(null);
        setCustomers((prev) =>
          customersDto.map((dto) => {
            const previous = prev.find((entry) => entry.id === dto.id || entry.email.toLowerCase() === dto.email.toLowerCase());
            return mapCustomerSummary(dto, previous);
          })
        );
        setOrders(ordersDto.map(mapOrder));
      } catch (error) {
  ...
  useEffect(() => {
    const status = customerProfile?.status ?? auth.status;
    if (status === "inactive") {
      if (!inactiveNoticeShownRef.current) {
        showNotification({
        ...
        inactiveNoticeShownRef.current = true;
      }
    } else {
      inactiveNoticeShownRef.current = false;
    }
  }, [customerProfile?.status, auth.status, showNotification]);
  ...
  const registerCustomer = useCallback<ContextValue["registerCustomer"]>(
    async (payload) => {
      try {
        const request = toCustomerRegistrationRequest(payload);
        ...
        await applyAuthState({
          ...
          status: normalizeAccountStatus(loginResult.status)
        });
  ...
  const createCustomerAsAdmin = useCallback<ContextValue["createCustomerAsAdmin"]>(
    async (payload) => {
      ...
      try {
        const request = toCustomerRegistrationRequest(payload);
        const profile = await apiRegisterCustomer(request);
        const mapped = mapCustomerProfile(profile);
        setCustomers((prev) => {
          const others = prev.filter((entry) => entry.email.toLowerCase() !== mapped.email.toLowerCase());
          return [...others, mapped].sort((a, b) => a.nombre.localeCompare(b.nombre, "es"));
        });
        return { ok: true };
      } catch (error) {
        return { ok: false, message: extractErrorMessage(error, "No se pudo registrar al cliente") };
      }
    },
  ...
  const upsertCustomer = useCallback<ContextValue["upsertCustomer"]>(
    async (user) => {
      ...
      try {
        ...
        setCustomers((prev) => {
          const previous = prev.find((entry) => entry.id === summaryDto.id || entry.email.toLowerCase() === summaryDto.email.toLowerCase());
          const others = prev.filter((entry) => entry.id !== summaryDto.id && entry.email.toLowerCase() !== summaryDto.email.toLowerCase());
          const snapshotSource = previous ?? user;
          return [...others, mapCustomerSummary(summaryDto, snapshotSource)].sort((a, b) => a.nombre.localeCompare(b.nombre, "es"));
        });
        return { ok: true };
      } catch (error) {
  ...
  const changeOrderStatus = useCallback<ContextValue["changeOrderStatus"]>(
    async (orderId, nextStatusLabel) => {
      ...
      try {
        ...
        setOrders((prev) => {
          const others = prev.filter((order) => order.id !== mapped.id);
          return [mapped, ...others].sort((a, b) => b.createdAt - a.createdAt);
        });
        return { ok: true };
      } catch (error) {
  ...
  const addComment = useCallback<ContextValue["addComment"]>(
    async (postSlug, text) => {
      ...
      if (customerProfile?.status === "inactive") {
        return { ok: false, message: "Tu cuenta está desactivada. No puedes comentar." };
      }
      ...
      const isProduct = postSlug.startsWith(PRODUCT_COMMENT_PREFIX);
      try {
        if (isProduct) {
          const productId = postSlug.slice(PRODUCT_COMMENT_PREFIX.length);
          if (!productId) {
            return { ok: false, message: "Producto no encontrado." };
          }
          const comment = await apiCreateProductReview(productId, content, auth.token);
          setComments((prev) => ({
            ...prev,
    ...
    [auth.token, auth.role, customerProfile?.status]
  );
  ...
  const editComment = useCallback<ContextValue["editComment"]>(
    async (postSlug, id, text) => {
      ...
      if (customerProfile?.status === "inactive") {
        return { ok: false, message: "Tu cuenta está desactivada. No puedes editar comentarios." };
      }
      const content = text.trim();
      if (!content) {
      ...
      try {
        ...
        const updated = isProduct
          ? await apiUpdateProductReview(id, content, auth.token)
          : await apiUpdateBlogComment(id, content, auth.token);
        setComments((prev) => {
    ...
    [auth.token, customerProfile?.status]
  );
  ...
  const deleteComment = useCallback<ContextValue["deleteComment"]>(
    async (postSlug, id) => {
      ...
      try {
        if (postSlug.startsWith(PRODUCT_COMMENT_PREFIX)) {
          await apiDeleteProductReview(id, auth.token);
        } else {
  ...
  const placeOrder = useCallback<ContextValue["placeOrder"]>(
    async (request) => {
      ...
      if (customerProfile?.status === "inactive") {
        return { ok: false, message: "Tu cuenta está desactivada. No puedes completar compras." };
      }
      try {
    ...
    [auth.token, auth.role, refreshProducts, customerProfile?.status]
  );
